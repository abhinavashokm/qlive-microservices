          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-8 mb-10">
            ...
            <Card className="flex flex-col">
              ...
              <form onSubmit={handleCreate} className="space-y-4 mt-auto">
                ...
                />
                <Button type="submit" variant="gradient" isLoading={isCreating} className="w-full">Create</Button>
              </form>
              ...
              {createdSession && (
                <div className="mt-6 p-6 bg-brand-500/10 border border-brand-500/20 rounded-2xl text-center animate-slide-up">
                  ...
                  <p className="text-4xl font-mono font-extrabold tracking-[0.2em] text-white mb-6 drop-shadow-lg">{createdSession.invite_code}</p>
                  <div className="space-y-3 flex flex-col">
                    <Button variant="secondary" onClick={() => copyToClipboard(createdSession.invite_code)} className="w-full text-sm py-3">
                      Copy Link
                    </Button>
                    <Button variant="gradient" onClick={() => navigate(ROUTES.SESSION(createdSession.invite_code))} className="w-full text-sm py-3">
                      Enter Room
                    </Button>
                  </div>
                </div>
            ...
            <Card className="flex flex-col">
              ...
              <form onSubmit={handleJoin} className="space-y-4 mt-auto">
                ...
                />
                <Button type="submit" variant="secondary" isLoading={isJoining} className="w-full">Join Room</Button>
              </form>
            </Card>
          </div>

          <Card>
            <h2 className="text-xl font-bold text-white mb-6">My Sessions</h2>
            {loadingSessions ? (
              ...
              <div className="text-center py-16 px-4 border-2 border-dashed border-white/10 rounded-2xl bg-zinc-900/30">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {sessions.map(s => (
                  <div 
                    ...
                    className="flex flex-col justify-center p-5 bg-zinc-900/50 border border-white/5 rounded-2xl hover:bg-zinc-800/80 hover:border-brand-500/30 cursor-pointer transition-all shadow-lg group relative overflow-hidden"
                  >
                    ...
                    <h3 className="font-bold text-zinc-100 text-lg group-hover:text-white mb-3 truncate relative z-10">{s.title}</h3>

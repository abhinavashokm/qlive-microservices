...
export default function QuestionCard({ question, isHost, answerInput, onAnswerInputChange, onSubmitAnswer, onMarkDone, onUpvote }) {
  ...
  return (
    <div className={`glass-panel p-4 sm:p-6 rounded-3xl transition-all duration-300 hover:border-white/10 ${question.is_answered ? 'opacity-70 bg-zinc-900/30' : 'bg-zinc-900/50'}`}>
      <div className="flex items-start gap-4 sm:gap-6">
          ...
          <button 
            ...
            className={`flex items-center justify-center w-12 h-12 rounded-2xl transition-all duration-300 ${
              hasUserVoted 
                ...
                : 'bg-zinc-800/50 text-zinc-400 hover:bg-zinc-700 hover:text-white border border-white/5'
            }`}
          >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex flex-wrap items-start justify-between gap-3 mb-2">
            <p className={`text-lg sm:text-xl font-medium leading-relaxed break-words ${question.is_answered ? 'text-zinc-400' : 'text-zinc-100'}`}>
              {question.text}
            </p>
          ...
          {question.answerText && (
            <div className="mt-4 bg-emerald-500/5 p-4 sm:p-5 rounded-2xl border border-emerald-500/10">
              ...
              <p className="text-zinc-300 text-sm sm:text-base leading-relaxed">{question.answerText}</p>
            </div>
          )}
            ...
            <div className="mt-6 pt-5 border-t border-white/5">
              <div className="flex flex-col sm:flex-row gap-3">
                <textarea
                  ...
                  className="flex-1 px-4 py-3 text-sm bg-zinc-950/50 border border-white/5 rounded-xl focus:outline-none focus:ring-2 focus:ring-brand-500/30 text-zinc-200 placeholder-zinc-600 resize-none min-h-[44px]"
                />
